...
/// </para>
/// <para>
/// 收口完成的判据不是这里归零——是渲染检查 <c>language</c> 套件的"英文界面零中文"总闸
...
public class I18nRulesTests
{
    ...
    /// </summary>
    /// <remarks>
    /// 本仓纪律：放宽/新增判据时必须留"它能红"的证据（同族见渲染检查 P9 的负向对照）。
    ...
    /// 渲染检查逐页扫可视树里的 <c>LocValue {</c> 文本（动态总闸），以及"文案值不许是 <c>string</c>"这条口径
    ...
    /// 改判据必须同时给出"它量得到东西"的证据（与渲染检查 P9 的负向对照同一条）。
    /// </remarks>
    [Fact]
