...
internal static partial class SmokeRunner
{
    ...
    // —— §0 目录自描述：70 命令（54 模块 + 16 编排）、分类、能力标志 ——
    private static Task SectionCatalog(SmokeState s)
    {
        var manifest = s.Client.Describe();
        Asserts.That(manifest.Commands.Count == 70, $"目录应有 70 条命令，实际 {manifest.Commands.Count}");
        Asserts.That(manifest.Commands.Select(c => c.Name).Distinct().Count() == 70, "命令名不得重复");
        ...
        Asserts.That(Count("folders") == 14 && Count("links") == 16 && Count("trash") == 8
        ...
        Asserts.That(queries.All(c => !c.IsMutation) && manifest.Commands.Count(c => c.IsMutation) == 70 - queries.Count,
            "Query/Mutation 互斥且每条命令必有其一");

        Console.WriteLine("[OK] §0 目录自描述：70 命令（54 模块 + 16 编排）/ 十二域 / 破坏性标志");
    ...
    private static async Task SectionWire(SmokeState s)
    {
        ...
        Asserts.That(result.GetProperty("commands").GetArrayLength() == 70, "describe 应含 70 条命令");
