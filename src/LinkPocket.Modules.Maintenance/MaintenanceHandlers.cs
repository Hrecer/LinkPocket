...
internal sealed class DiagnosticsCollectHandler(Func<EngineRuntimeStats>? runtimeStats) : ICommandHandler
{
    ...
    public async Task<CommandResult> ExecuteAsync(ICommandContext ctx, JsonElement args)
    {
        ...
        var diagnostics = new
        {
            ...
            counts = new
            {
                // 只计数，绝不把整表拉进内存（此前 folders/trash 全量 SELECT）
    ...
    private static string GetAppVersion()
        // 模块程序集版本才是「LinkPocket 的版本」——GetEntryAssembly 在测试/工具宿主下
        // 会返回宿主版本，语义漂移
...
internal sealed class MaintenanceReinitHandler : ICommandHandler
{
    ...
    public async Task<CommandResult> ExecuteAsync(ICommandContext ctx, JsonElement args)
    {
        ...
        // dryRun 必须零副作用：文件系统操作不受事务保护，预演时跳过。
        // Task.Run：Directory.Delete(递归) 是同步 IO，避免在 UI 线程续体上卡住。
        var faviconCleared = ctx.DryRun
            ? false
            : await Task.Run(() => TryClearFaviconCache(), ctx.Ct);

        return CommandResult.Ok(
            // 强类型 DTO 落形（JsonElement 承载——既有 ExecuteAsync<JsonElement> 调用
