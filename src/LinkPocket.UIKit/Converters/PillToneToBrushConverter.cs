...
/// 图标色按"工具栏同色"取：**还原 / 还原到根目录 / 跳转 = 强调填充档**（与工具栏主按钮同一填充色）；
/// 破坏性动作 = 容器字档（与次操作共用同一套呈现，不红不黄）。
/// ⚠️ 不取主题 `Primary` 亮紫（#6750A4）：在 TintCard 上比强调填充更刺眼、与工具栏按钮不同色。
/// 也不取 `SecondaryContainer`：那是极浅的填充色，做描边几乎不可见。
/// </summary>
/// <remarks>
