    ...
    public partial class BackupPanel : UserControl
    {
        ...
        private void ShowOverlay(Border overlay, string message, int current, int total)
        {
            ...
            if (bar != null)
            {
                ...
                bar.SetResourceReference(WavyProgressBar.ActiveBrushProperty, Theming.Tokens.AppTokens.AccentFill);
                bar.Value = 0;
                bar.Maximum = 100;
            }
        ...
        private static int ReadCount(JsonElement? data, string property, out bool known)
        {
            ...
            known = false;
            LpLog.Error($"backup import result is missing the count field {property} (engine backup.import must always return it)", null);
            return 0;
        }
    }
}
