...
internal static class BackupIO
{
    ...
    /// <remarks>
    /// <para>
    /// 用户令（2026-09-20）："确认我们这次的备份格式没有缺陷，然后定稿这份备份格式，
    /// **取一个版本标识名**，作为定稿（以后仍然可能会改，所以我们要取一个版本标识名）"。
    /// </para>
    /// <para>
    ...
    /// <remarks>
    /// <para>
    /// <b>原子落盘（用户令 2026-09-20："确保数据是安全的"）</b>：整包先写到**同目录临时文件**
    ...
    //
    // 用户令 2026-09-20（"确保数据是安全的"）：备份文件是**外部输入**，没有上限就等于把进程的内存
    ...
    /// <remarks>
    /// <para>
    /// <b>为什么要比对而不是"存在就跳过"</b>（用户令 2026-09-20："确保数据是安全的"）：本地缓存可能属于
    ...
    //
    // 用户令 2026-09-20（"确保数据是安全的"）：旧实现的 `ParseDateTime` 在解析失败时**静默回落 DateTime.UtcNow**
