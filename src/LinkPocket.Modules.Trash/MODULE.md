...
- 还原**缺省**回删除前位置（`to = "origin"`，2026-09-19 定稿）；原位置快照 = 链接的 `origin_folder_id`（既有）
...
- `ModulesTests.cs` → `TrashModuleTests`（两阶段确认 / 单元子树清除 / **混合批量还原矩阵：origin 缺省与回落 / root 显式 / C2 落回同批单元 / 批内同名编号 / 同 URL 重复计数 / 空批量与坏数据硬失败回滚** / **搬移：移入-移出-成环拒绝**）
